			...
			void *data)
{
	...
	case RADIUS_CODE_ACCESS_ACCEPT:
		if (sta->ssid->dynamic_vlan == DYNAMIC_VLAN_DISABLED){
			...
			sta->identity = dup_binstr(identity, identity_len);
			sm->identity_len = identity_len;
			...
			json_object *c_mac = json_object_new_string(com_mac);

			json_object_object_add(inside,"name", c_identity);
			json_object_object_add(inside,"id", c_mac);

			//printf ("The json object created: %s \n",json_object_to_json_string(inside));

			json_object *user = json_object_new_object();
			json_object_object_add(user,"network", inside);
			...
			if(curl) {
			    ...
			    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, json_object_to_json_string(user));
