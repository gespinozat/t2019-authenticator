...
struct radius_tunnel_attrs {
	...
	int vlanid;
	// G.E.T
	char *communities; // pointer to communities
};
...
/**
 * G.E.T.
 * radius_msg_get_communities
 * @msg: RADIUS message
 * Returns: pointer to communities ("string" received from RADIUS)


void radius_msg_get_communities(struct radius_msg *msg, char txt[])
{
	struct radius_tunnel_attrs tunnel[RADIUS_TUNNEL_TAGS], *tun;
	size_t i;
	struct radius_attr_hdr *attr = NULL;
	...
	for (i = 0; i < msg->attr_used; i++) {
		...
		case RADIUS_ATTR_TUNNEL_PRIVATE_GROUP_ID:
			...
			buf[dlen] = '\0';
			tun->tag_used++;
			tun->communities = buf;  // REVISAR, POSIBLEMENTE GENERARA ERROR AL COMPILAR 30-10-2017
			break;
		}
	}
	//os_memcpy(txt, buf, sizeof(buf));
	os_memcpy(txt, buf, os_strlen(buf)+1);
